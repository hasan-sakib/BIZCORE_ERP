...
<div class="d-flex align-items-center justify-content-between mb-4">
    <a href="/sales/quotations/<?= (int)$quotation['id'] ?>" class="btn btn-sm btn-outline-secondary">
...
<div class="card shadow-sm">
    <div class="card-header bg-white border-bottom">
        <h5 class="mb-0">
            <i class="fas fa-file-alt me-2 text-primary"></i>Edit Quotation
            <span class="badge bg-secondary ms-2"><?= sanitize($quotation['reference_no'] ?? '') ?></span>
        </h5>
    </div>
    <div class="card-body">
        ...
        <form method="POST" action="/sales/quotations/<?= (int)$quotation['id'] ?>" id="quotation-form">
            <?= csrf_field() ?>
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" name="issue_date" class="form-control <?= isset($errors['issue_date']) ? 'is-invalid' : '' ?>"
                           value="<?= sanitize($old['issue_date'] ?? '') ?>" required>
                ...
                <div class="col-md-3">
                    ...
                    <input type="date" name="expiry_date" class="form-control <?= isset($errors['expiry_date']) ? 'is-invalid' : '' ?>"
                           value="<?= sanitize($old['expiry_date'] ?? '') ?>">
                    <?php if (isset($errors['expiry_date'])): ?>
                        <div class="invalid-feedback"><?= sanitize(implode(' ', (array)$errors['expiry_date'])) ?></div>
                    <?php endif; ?>
                </div>
                    ...
                    <tbody id="items-container">
                        ...
                        <tr class="item-row">
                            ...
                            <td><input type="text" name="items[<?= $idx ?>][description]" class="form-control form-control-sm"
                                       value="<?= sanitize($item['description'] ?? '') ?>" placeholder="Description"></td>
                            <td><input type="number" name="items[<?= $idx ?>][quantity]" class="form-control form-control-sm item-qty"
                                       step="0.0001" min="0" value="<?= (float)($item['quantity'] ?? 1) ?>"></td>
                            <td><input type="number" name="items[<?= $idx ?>][unit_price]" class="form-control form-control-sm item-price"
                                       step="0.0001" min="0" value="<?= (float)($item['unit_price'] ?? 0) ?>"></td>
                            <td class="text-end">
                                <span class="line-total fw-semibold"><?= number_format((float)($item['total'] ?? 0), 2) ?></span>
                            </td>
                ...
                <div class="col-md-4">
                    <table class="table table-sm">
                        <tr>
                            ...
                            <td class="text-end fw-semibold" id="subtotal"><?= number_format((float)($quotation['subtotal'] ?? 0), 2) ?></td>
                        </tr>
                        <tr>
                            <td class="text-muted">Tax</td>
                            <td class="text-end" id="tax-amount"><?= number_format((float)($quotation['tax_amount'] ?? 0), 2) ?></td>
                        </tr>
                        <tr class="fw-bold border-top">
                            <td>Total</td>
                            <td class="text-end text-primary fs-5" id="grand-total"><?= number_format((float)($quotation['total_amount'] ?? 0), 2) ?></td>
                        </tr>
                    </table>
                </div>
            </div>
            ...
            <div class="d-flex gap-2 justify-content-end">
                <a href="/sales/quotations/<?= (int)$quotation['id'] ?>" class="btn btn-secondary">Cancel</a>
